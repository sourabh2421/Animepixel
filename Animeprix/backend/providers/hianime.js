...
const PROVIDER_NAME = 'hianime';

/** HiAnime adapter — Consumet API provider path "hianime". No shared parsing. */
export class HiAnime extends AnimeProvider {
  ...
  async search(query) {
    ...
    if (res.status >= 400) throw new Error(`HiAnime search failed: ${res.status}`);
    const raw = res.data;
  ...
  async getAnime(id) {
    ...
    if (res.status >= 400) throw new Error(`HiAnime getAnime failed: ${res.status}`);
    const d = res.data;
  ...
  async getStream(episodeId) {
    ...
    const res = await axios.get(url, { headers: this.headers, timeout: 30000, validateStatus: () => true });
    if (res.status >= 400) throw new Error(`HiAnime getStream failed: ${res.status}`);
    const d = res.data;
    const sources = [
