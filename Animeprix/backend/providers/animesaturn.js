...
const PROVIDER_NAME = 'animesaturn';

/** AnimeSaturn adapter — Consumet API provider path "animesaturn". No shared parsing. */
export class AnimeSaturn extends AnimeProvider {
  ...
  async search(query) {
    ...
    if (res.status >= 400) throw new Error(`AnimeSaturn search failed: ${res.status}`);
    const raw = res.data;
  ...
  async getAnime(id) {
    ...
    if (res.status >= 400) throw new Error(`AnimeSaturn getAnime failed: ${res.status}`);
    const d = res.data;
  ...
  async getStream(episodeId) {
    ...
    const res = await axios.get(url, { headers: this.headers, timeout: 30000, validateStatus: () => true });
    if (res.status >= 400) throw new Error(`AnimeSaturn getStream failed: ${res.status}`);
    const d = res.data;
    const sources = [
