...
/** AnimeKai adapter — Consumet API provider path "animekai". No shared parsing. */
export class AnimeKai extends AnimeProvider {
  ...
  async search(query) {
    ...
    if (res.status >= 400) throw new Error(`AnimeKai search failed: ${res.status}`);
    const raw = res.data;
  ...
  async getAnime(id) {
    ...
    if (res.status >= 400) throw new Error(`AnimeKai getAnime failed: ${res.status}`);
    const d = res.data;
  ...
  async getStream(episodeId) {
    ...
    const res = await axios.get(url, { headers: this.headers, timeout: 30000, validateStatus: () => true });
    if (res.status >= 400) throw new Error(`AnimeKai getStream failed: ${res.status}`);
    const d = res.data;
    const sources = [
