...
const PROVIDER_NAME = 'zoro';

/** Zoro adapter — Consumet API provider path "zoro". No shared parsing. */
export class Zoro extends AnimeProvider {
  ...
  async search(query) {
    ...
    if (res.status >= 400) throw new Error(`Zoro search failed: ${res.status}`);
    const raw = res.data;
  ...
  async getAnime(id) {
    ...
    if (res.status >= 400) throw new Error(`Zoro getAnime failed: ${res.status}`);
    const d = res.data;
  ...
  async getStream(episodeId) {
    ...
    const res = await axios.get(url, { headers: this.headers, timeout: 30000, validateStatus: () => true });
    if (res.status >= 400) throw new Error(`Zoro getStream failed: ${res.status}`);
    const d = res.data;
    const sources = [
