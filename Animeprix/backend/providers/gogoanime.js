...
/** Gogoanime adapter — Consumet API provider path "gogoanime". No shared parsing. */
export class GogoAnime extends AnimeProvider {
  ...
  async search(query) {
    ...
    if (res.status >= 400) throw new Error(`Gogoanime search failed: ${res.status}`);
    const raw = res.data;
  ...
  async getAnime(id) {
    ...
    if (res.status >= 400) throw new Error(`Gogoanime getAnime failed: ${res.status}`);
    const d = res.data;
  ...
  async getStream(episodeId) {
    ...
    const res = await axios.get(url, { headers: this.headers, timeout: 30000, validateStatus: () => true });
    if (res.status >= 400) throw new Error(`Gogoanime getStream failed: ${res.status}`);
    const d = res.data;
    const sources = [
