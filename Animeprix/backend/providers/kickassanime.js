...
const PROVIDER_NAME = 'kickassanime';

/** KickAssAnime adapter — Consumet API provider path "kickassanime". No shared parsing. */
export class KickAssAnime extends AnimeProvider {
  ...
  async search(query) {
    ...
    if (res.status >= 400) throw new Error(`KickAssAnime search failed: ${res.status}`);
    const raw = res.data;
  ...
  async getAnime(id) {
    ...
    if (res.status >= 400) throw new Error(`KickAssAnime getAnime failed: ${res.status}`);
    const d = res.data;
  ...
  async getStream(episodeId) {
    ...
    const res = await axios.get(url, { headers: this.headers, timeout: 30000, validateStatus: () => true });
    if (res.status >= 400) throw new Error(`KickAssAnime getStream failed: ${res.status}`);
    const d = res.data;
    const sources = [
